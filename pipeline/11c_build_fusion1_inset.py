#!/usr/bin/env python3
"""11_build_fusion1.py -- build the 5 m Fusion1 INSET over the dense-Garmin reef strip.
...
# Smooth the low-confidence tiers (GMRT deep-fill=1, nearest-neighbour hull-interp=2) — see
# pipeline/11_build_fusion1.py for the full rationale: 1-NN fill is a Voronoi assignment with
# sharp cliffs at cell-ownership boundaries, not real seafloor structure. Only low-confidence
# cells are replaced; every measured tier (3-6) is left byte-for-byte unchanged.
...
write(OUT/'fusion1_inset_depth_4326.tif',depth_o,'float32',nod)
write(OUT/'fusion1_inset_uncertainty_4326.tif',unc_o,'float32',nod)
write(OUT/'fusion1_inset_source_4326.tif',source,'uint8',0)
